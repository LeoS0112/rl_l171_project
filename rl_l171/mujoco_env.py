...
def _load_scene(nr_cubes):
    ...
    for i in range(nr_cubes):
        ...
        xml.append(MujocoXML.parse(letter_xml).add_name_prefix(f"cube_{i}:"))
...
class CubesMujocoSim:
    ...
    def __init__(self, randomise_initial_position=False, seed=None, nr_cubes=5):
        ...
        self.qpos_arm = self.data.qpos[
            self.base_dofs : (self.base_dofs + self.arm_dofs)
        ]
        qpos_arm = self.data.qpos[self.base_dofs : (self.base_dofs + self.arm_dofs)]
        self.qvel_arm = self.data.qvel[
            self.base_dofs : (self.base_dofs + self.arm_dofs)
        ]
        ctrl_arm = self.data.ctrl[self.base_dofs : (self.base_dofs + self.arm_dofs)]
        self.qpos_gripper = self.data.qpos[
            (self.base_dofs + self.arm_dofs) : (
                self.base_dofs + self.arm_dofs + gripper_dofs
            )
        ]
        ...
        self.arm_controller = CamralArmController(
            qpos_arm,
            self.qvel_arm,
            ctrl_arm,
            self.qpos_gripper,
    ...
    def reset(self, seed=None):
        ...
        cube_quaternions = [
            ...
            [
                0.9999999999371122,
                2.4507530841173426e-18,
                -2.1775598762200762e-19,
                -1.1214989255568561e-05,
            ],
        ]
    ...
    def _update_controls(self, command):
        if command is not None:
            assert "arm_pos_global" in command and "base_pose" in command, (
                "Command must include 'arm_pos_global' and 'base_pos'"
            )
            ...
            target_changed = (
                (
                    ...
                    or np.linalg.norm(
                        target_arm_pos_global - self.last_arm_target_pos_global
                    )
                    > POSITION_DEADBAND
                )
                or (
                    ...
                    or np.linalg.norm(
                        target_arm_quat_global_xyzw - self.last_arm_target_quat_global
                    )
                    > ORIENTATION_DEADBAND
                )
                or (
                    self.last_base_target_pos is None
                    or np.linalg.norm(target_base_pose - self.last_base_target_pos)
                    > POSITION_DEADBAND
                )
                or (
                    self.last_gripper_pos is None
                    or np.linalg.norm(target_gripper_pos - self.last_gripper_pos)
                    > POSITION_DEADBAND
                )
            )
            ...
            if target_changed:
                ...
                mujoco.mju_rotVecQuat(
                    target_arm_pos, tmp, self.base_quat_inv
                ...
                mujoco.mju_mulQuat(
                    target_local_quat_wxyz, self.base_quat_inv, target_global_quat_wxyz
                )
                target_arm_quat_xyzw = target_local_quat_wxyz[
                    [1, 2, 3, 0]
                ]  # back to (x,y,z,w) as IK piper expects this
